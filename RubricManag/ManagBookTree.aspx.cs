	...
	public partial class ManagBookTree : System.Web.UI.Page
	{
		...
		protected void ButDel_Click(object sender, System.EventArgs e)
		{
			...
			if (depth==0)
			{
                if ((myLoginID.Trim().ToUpper() == "ADMIN") || (CreateUserID == myUserID.Trim()))
                {
                    string sql = "select count(*) from ChapterInfo where SubjectID =" + subjectid;
                    SqlCmd = new SqlCommand(sql, SqlConn);
                    object o = SqlCmd.ExecuteScalar();
                    int count = Convert.ToInt16(o);
                    if (count== 0)
                    {
                        string sqlDeleteSubject = "delete from SubjectInfo where SubjectID =" + subjectid;
                        SqlCmd = new SqlCommand(sqlDeleteSubject, SqlConn);
                        SqlCmd.ExecuteNonQuery();
                        this.RegisterStartupScript("newWindow", "<script language='javascript'>alert('成功删除科目！');</script>");
                       
                    }
                    else {
                        this.RegisterStartupScript("newWindow", "<script language='javascript'>alert('请先删除相应的章节！');</script>");
