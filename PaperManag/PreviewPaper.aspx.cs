					...
					if (intPaperID!=0)
					{
						...
						for(int i=0;i<SqlDSTestType.Tables["PaperTestType"].Rows.Count;i++)
						{
							...
							for(int j=0;j<SqlDSTest.Tables["PaperTest"].Rows.Count;j++)
							{
								...
								strPaperContent=strPaperContent+"<tr>";
								strPaperContent=strPaperContent+"<td colspan='4' width='100%'>"+intTestNum.ToString()+"."+SqlDSTest.Tables["PaperTest"].Rows[j]["TestContent"].ToString()+"<font color='red'>（"+SqlDSTest.Tables["PaperTest"].Rows[j]["TestMark"].ToString()+"分）</font></td>";
								strPaperContent=strPaperContent+"</tr>";
								if (SqlDSTestType.Tables["PaperTestType"].Rows[i]["BaseTestType"].ToString()=="单选类")
								{
									...
									for(k=1;k<=intOptionNum;k++)
									{
										...
										strPaperContent=strPaperContent+"<td width='25%'>"+Convert.ToChar(64+k)+"."+strArrOptionContent[k-1]+"</td>";
